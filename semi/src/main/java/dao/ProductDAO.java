...
public class ProductDAO {
	...
	public TvVO getTvInfo(String pname) {
		...
		sb.setLength(0);
		sb.append("SELECT pno, code, pname, pdesc, brand, price, to_char(regdate, 'yyyy-mm-dd') regdate, energy, imgfile, psize, pixel ");
		sb.append("FROM tv ");
		...
		try {
			...
			vo = new TvVO(pno, code, pname, pdesc, brand, price, regdate, energy, imgfile, psize, pixel);
			conn.close();
	...
	public void setTvCompare(TvVO vo) {
		...
		sb.append("VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ) ");
		sb.append("WHERE count(pno) > ( SELECT count(pno) ");
		sb.append("					    FROM compare_tv ");
		sb.append("					    WHERE count(pno) < 5 ) ");
	...
	public ArrayList<TvVO> getAll() {
		...
		sb.setLength(0);
		sb.append("SELECT pno, code, pname, pdesc, brand, price, regdate, energy, imgfile, psize, pixel ");
		sb.append("FROM compare_tv ");
		...
		try {
			...
			while(rs.next()) {
				...
				TvVO vo = new TvVO(pno, code, pname, pdesc, brand, price, regdate, energy, imgfile, psize, pixel);
				list.add(vo);
			}
			conn.close();
		} catch (SQLException e) {
